use crate::{stream::{IndexedChar, Stream}, binders::StreamBinder, output::Output, character::UnconsSpec};
    ...
    fn bind_to<'a>(self, stream: Stream<'a>) -> Output<'a, Self::Ok<'a>, Self::Err> {
        stream.static_alternatives(&[
            &|stream: Stream<'a>| -> Output<'a, InSomeEnclosure<'a>, ()> {
                stream.apply_binder(ParseInCurlyBrackets::default()).ok_map(|x| InSomeEnclosure::CurlyBrackets(x))
            },
            &|stream: Stream<'a>| -> Output<'a, InSomeEnclosure<'a>, ()> {
                stream.apply_binder(ParseInSquareBrackets::default()).ok_map(|x| InSomeEnclosure::SquareBrackets(x))
            },
            &|stream: Stream<'a>| -> Output<'a, InSomeEnclosure<'a>, ()> {
                stream.apply_binder(ParseInRoundBrackets::default()).ok_map(|x| InSomeEnclosure::RoundBrackets(x))
            },
            &|stream: Stream<'a>| -> Output<'a, InSomeEnclosure<'a>, ()> {
                stream.apply_binder(ParseInAngleBrackets::default()).ok_map(|x| InSomeEnclosure::AngleBrackets(x))
            },
        ])
    }
}
