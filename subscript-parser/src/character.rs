...
use crate::{stream::{Cursor, IndexedChar, Stream}, binders::StreamBinder, token::TokenView, output::{Output, IO}};
...
impl CharView {
    pub fn is_first_char(&self) -> bool { self.span_index == 0 }
}
...
impl CharPredicate for () {
    fn satisfies(&self, view: CharView) -> bool {true}
}
...
impl CharPredicate for StaticCharPredicate {
    fn satisfies(&self, view: CharView) -> bool { self.is_valid(view) }
}
        ...
        for (ix, char) in stream.slice.into_iter().enumerate() {
            ...
            if !self.is_valid(view) {
                ...
                return Output::success(IO {
                    ...
                    value: Stream{slice: value, cursor: stream.cursor}.to_token_view(),
                });
            }
        }
...
impl StaticCharPredicate {
    pub const IS_WHITESPACE: StaticCharPredicate = {
        fn predicate(view: CharView) -> bool { view.value.char == ' ' }
    ...
    pub const IS_LETTER: StaticCharPredicate = {
        fn predicate(view: CharView) -> bool { view.value.char.is_alphabetic() }
        StaticCharPredicate::new(&predicate)
    };
    pub const IS_NUMBER: StaticCharPredicate = {
        fn predicate(view: CharView) -> bool { view.value.char.is_numeric() }
        StaticCharPredicate::new(&predicate)
    };
    pub const IS_LETTER_OR_NUMBER: StaticCharPredicate = {
        fn predicate(view: CharView) -> bool { view.value.char.is_alphanumeric() }
        StaticCharPredicate::new(&predicate)
    };
...
impl TakeWhileSpec<StaticCharPredicate> {
    pub const IDENTIFIER: Self = {
        TakeWhileSpec {
            ...
            ignoring: Some(StaticCharPredicate::IS_WHITESPACE),
        }
    };
    pub const SYMBOL: Self = {
        TakeWhileSpec { while_true: StaticCharPredicate::IS_SYMBOL, ignoring: None}
    };
    pub const NOT_SYMBOL: Self = {
        TakeWhileSpec { while_true: StaticCharPredicate::IS_NOT_SYMBOL, ignoring: None}
    };
    pub const WHITE_SPACE: Self = {
        TakeWhileSpec { while_true: StaticCharPredicate::IS_WHITESPACE, ignoring: None}
    };
    pub const NEWLINE: Self = {
        TakeWhileSpec { while_true: StaticCharPredicate::IS_NEWLINE, ignoring: None}
    };
    pub const NOT_NEWLINE: Self = {
        TakeWhileSpec { while_true: StaticCharPredicate::IS_NOT_NEWLINE, ignoring: None}
    };
}
...
impl<P: CharPredicate + 'static> StreamBinder for UnconsSpec<P> {
    ...
    fn bind_to<'a>(self, stream: Stream<'a>) -> Output<'a, Self::Ok<'a>, Self::Err> {
        ...
        let is_valid = self.filter.map_or(true, |p| p.satisfies(CharView {
            span_index: 0,
            start_cursor: stream.cursor,
            value: head,
        }));
